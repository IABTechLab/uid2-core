...
public class TestCoreVerticle {
    ...
    void s3encryptionKeyRetrieveNoKeysOrError(Vertx vertx, VertxTestContext testContext) {
        ...
        when(s3KeyProvider.getMostRecentKeysForSite(anyInt())).thenReturn(Collections.emptyList());
        ...
        get(vertx, "s3encryption_keys/retrieve", ar -> {
            if (ar.succeeded()) {
                ...
                when(s3KeyProvider.getMostRecentKeysForSite(anyInt())).thenThrow(new RuntimeException("Test exception"));
