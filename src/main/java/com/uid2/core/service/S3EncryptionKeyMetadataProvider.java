...
public class S3EncryptionKeyMetadataProvider implements IS3EncryptionKeyMetadataProvider {
    private final ICloudStorage metadataStreamProvider;
    private final ICloudStorage downloadUrlGenerator;

    public S3EncryptionKeyMetadataProvider(ICloudStorage cloudStorage) {
    ...
    public String getMetadata(OperatorInfo info) throws Exception {
        String pathname = getMetadataPathName(info.getOperatorType(), info.getSiteId(), SecretStore.Global.get(Const.Config.S3keysMetadataPathProp));
        ...
        JsonObject obj = main.getJsonObject("s3encryption_keys");
